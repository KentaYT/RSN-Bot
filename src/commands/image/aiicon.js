...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("ai-icon")
    .setDescription("Generate a icon response based on a prompt")
    .addStringOption((option) =>
      option
        .setName("prompt")
        .setDescription("The main prompt for ai-icon")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("negative_prompt")
        .setDescription("The negative prompt for ai-icon")
        .setRequired(false)
    ),
  async execute(interaction) {
    ...
    try {
      ...
      const response = await rsnchat.icon(prompt, negativePrompt);
