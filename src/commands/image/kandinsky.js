...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("kandinsky")
    .setDescription("Generate a kandinsky response based on a prompt")
    .addStringOption((option) =>
      option
        .setName("prompt")
        .setDescription("The main prompt for kandinsky")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("negative_prompt")
        .setDescription("The negative prompt for kandinsky")
        .setRequired(false)
    ),
  async execute(interaction) {
    ...
    try {
      ...
      const response = await rsnchat.kandinsky(prompt, negativePrompt);
