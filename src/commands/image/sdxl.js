...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("sdxl")
    .setDescription("Generate a sdxl response based on a prompt")
    .addStringOption((option) =>
      option
        .setName("prompt")
        .setDescription("The main prompt for sdxl")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("negative_prompt")
        .setDescription("The negative prompt for sdxl")
        .setRequired(false)
    ),
  async execute(interaction) {
    ...
    try {
      ...
      const response = await rsnchat.sdxl(prompt, negativePrompt);
