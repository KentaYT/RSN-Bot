...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("dalle")
    .setDescription("Generate a dalle response based on a prompt")
    .addStringOption((option) =>
      option
        .setName("prompt")
        .setDescription("The main prompt for dalle")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("negative_prompt")
        .setDescription("The negative prompt for dalle")
        .setRequired(false)
    ),
  async execute(interaction) {
    ...
    try {
      ...
      const response = await rsnchat.dalle(prompt, negativePrompt);
